...
def main():
    ...
    # Update config/strategy_params.json if approved
    if final_decision['for_ml_layer']:
        ...
        config[args.symbol] = {
            **recommended_params,
            ...
            "_walk_forward_sharpe": sum(w['test_metrics']['sharpe_ratio'] for w in walkforward_data['windows']) / len(walkforward_data['windows']),
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"✓ UPDATED CONFIG FILE")
        logger.info(f"{'='*80}")
        logger.info(f"Config file: {config_path}")
        logger.info(f"Symbol: {args.symbol}")
        logger.info(f"Status: {final_decision['decision']}")
        logger.info(f"{'='*80}")

    else:
        logger.warning(f"\n{'='*80}")
        logger.warning(f"✗ CONFIG NOT UPDATED")
        logger.warning(f"{'='*80}")
        logger.warning(f"Decision: {final_decision['decision']}")
        logger.warning(f"Reason: Not approved for ML layer")
        logger.warning(f"{'='*80}")
    ...
    if final_decision['for_ml_layer']:
        ...
        logger.info(f"  2. Run data extraction with optimized parameters:")
        logger.info(f"     python research/extract_training_data.py --symbol {args.symbol}")
        logger.info(f"  3. Train ML model:")
        logger.info(f"     python research/train_rf_three_way_split.py --symbol {args.symbol}")
    else:
        logger.info(f"\nRecommendation:")
        logger.info(f"  Parameters did not pass all checks.")
        logger.info(f"  Review the evaluation reports and consider:")
        logger.info(f"  - Adjusting optimization parameter ranges")
        logger.info(f"  - Using different time periods")
        logger.info(f"  - Investigating underlying data quality issues")
