...
# FIX #2: Update embargo period in optimizer argument
...
# FIX #3: Replace held-out test evaluation - using Python for complex replacement
echo "Applying Fix #3: Fix held-out test evaluation..."
python3 << 'PYTHON_EOF'
# Read the file
with open('research/ml_meta_labeling/ml_meta_labeling_optimizer.py', 'r') as f:
    lines = f.readlines()

# Find the section to replace (look for "# Calculate held-out metrics")
start_idx = None
end_idx = None

for i, line in enumerate(lines):
    if '# Calculate held-out metrics' in line and i > 250:  # Make sure we're past line 250
        start_idx = i
    if start_idx is not None and '# Save held-out results' in line and i > start_idx:
        end_idx = i
        break
...
echo ""
echo "Next steps:"
echo "  1. python research/ml_meta_labeling/ml_meta_labeling_optimizer.py --symbol ES"
echo ""
